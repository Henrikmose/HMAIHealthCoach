...
async function lookupFood(foodName) {
  ...
  // ═══ [v112 KEYWORDS] EXACT KEYWORD STAGE — candidate selection fixed at source ═══
  // Per-row translations generated by Opus (keywords_version 1) with a ZERO-COLLISION
  // guarantee from the ambiguity checker: every phrase maps to exactly one food row.
  // Fixes the prefix-match disease ("pork loin" -> backribs; "chicken thighs" ->
  // frozen breaded) by making the right row a candidate BEFORE any scoring runs.
  // Placement: BEFORE the composed-dish guard — an exact keyword hit is by definition
  // a known single food, however many words it has ("frozen breaded chicken thigh").
  // Qualifier rule: if the user gave qualifiers ("fried", "with skin"), only a keyword
  ...
  try {
    ...
    if (kwHits && kwHits.length > 0) {
      ...
      const ranked = kwHits
        .map(r => ({ r, len: Math.max(0, ...kwCands.filter(c => (r.keywords || []).includes(c)).map(c => c.length)) }))
        .filter(x => x.len > 0)
...
function segmentMeals(message, currentHour = 12) {
  // [v93] Handles BOTH natural orientations:
...
// ═══ [v93] CODE-OWNED FOOD-LOG PIPELINE ═════════════════════════════════════
...
const ZERO_CAL_OK = /(water|black coffee|coffee, black|diet (soda|coke|pepsi)|sparkling water|seltzer|(green|black|herbal) tea|^tea\b|espresso)/i;

// Resolve a whole segment: code parser first; AI parse ONLY if code found nothing.
async function resolveSegment(segText) {
  // [v93] LABEL DECLARATION check FIRST: if this segment is "food + macro values",
  ...
  if (decl) {
    ...
    console.log(`  [v93] LABEL DECLARATION: "${decl.name}" ${decl.perServing.calories} cal (write-back: ${wb.action})`);
  ...
  for (const it of parsed.slice(0, 6)) {
    ...
    if (r && r.calories < 1 && r.protein < 0.5 && r.carbs < 0.5 && r.fat < 0.5
        && !ZERO_CAL_OK.test(r.food || "") && !ZERO_CAL_OK.test(it.food || "")) {
      unresolved.push(it.food);
      continue;
    }
...
const FACT_KINDS = {
  ...
  love: 2, dislike: 2, lifestyle: 2, nutrient: 2, activity: 2,
  constraint: 3,
};
...
function normalizeFactBlock(o) {
  ...
  // [v93] BATCH form: {"kind":"love","values":[...]} — one card for a whole pantry
...
// ═══ [v93] GOAL MATH — STRICT MODE. The delta method is DEAD. ════════════════
...
function computeGoalTargets(profile, currentGoals, params) {
  ...
  // [v93] REQUESTED TARGET: the user's stated calorie number is honored — up to the
  ...
  let targetNote = null;
  const requested = Number(params.target_calories) || null;
  if (requested && dir <= 0) {
    const requestedRate = (maintenance - requested) * 7 / 3500;
    if (requestedRate > MAX_RATE_LBS_WK) {
      calories = Math.round(maintenance - MAX_RATE_LBS_WK * 3500 / 7);
      ratePerWeek = MAX_RATE_LBS_WK;
      clamped = true;
      targetNote = `You asked for ~${requested} cal/day — that's ${Math.round(requestedRate * 100) / 100} lb/week, past the safe limit. The closest safe target is ${calories} (1.5 lb/week).`;
    } else if (requestedRate >= 0) {
      calories = requested;
      ratePerWeek = Math.round(requestedRate * 100) / 100;
      targetNote = `Using your requested ~${requested} cal/day (≈${ratePerWeek} lb/week).`;
    } else {
      targetNote = `Your requested ${requested} cal/day is above your maintenance (~${maintenance}) — that gains weight, so I kept a deficit instead.`;
    }
  ...
  // [v93] high-protein emphasis shifts the split (fat-loss default 35/35/30 -> 40/30/30)
  ...
  const ratios = dir > 0 ? (hiP ? { p: 0.35, c: 0.40, f: 0.25 } : { p: 0.30, c: 0.45, f: 0.25 })
              ...
              :           (hiP ? { p: 0.30, c: 0.40, f: 0.30 } : { p: 0.25, c: 0.45, f: 0.30 });
  ...
  let note = targetNote;
  if (dir < 0 && currentCalories && currentCalories < calories && !targetNote) {
  ...
  } else if (dir < 0 && currentCalories && currentCalories === calories && !targetNote) {
...
// ═══ [v93] GOAL STATEMENT PARSER — code understands "I want to lose 30 lbs in
...
function parseGoalStatement(text) {
  ...
  const wk = t.match(/(?:in|over|within)\s+(?:the\s+next\s+)?(\d+(?:\.\d+)?)\s*(weeks?|months?)\b/);
  if (wk) {
    const n = Number(wk[1]);
    timeframe_weeks = wk[2].startsWith("month") ? Math.round(n * 4.345) : n;
  } else {
    // [v93] word timeframes — "the next couple of months" defaulted to 1 lb/week before
    const wordSpans = [
      [/\b(a\s+couple\s+(?:of\s+)?months?|next\s+couple\s+(?:of\s+)?months?|couple\s+(?:of\s+)?months?)\b/, 9],
      [/\b(a\s+few\s+months?|few\s+months?)\b/, 13],
      [/\b(a\s+month|one\s+month|next\s+month)\b/, 4],
      [/\b(a\s+couple\s+(?:of\s+)?weeks?|couple\s+(?:of\s+)?weeks?)\b/, 2],
      [/\b(a\s+few\s+weeks?|few\s+weeks?)\b/, 3],
    ];
    for (const [re, weeks] of wordSpans) {
      if (re.test(t)) { timeframe_weeks = weeks; break; }
    }
  }

  // [v93] REQUESTED TARGET CALORIES ("I'd like to be around 2000"). Every candidate
  ...
  {
    const calRe = /\b([12]\d{3})\b(\s*(?:cal(?:orie)?s?|kcal))?/g;
    let best = null, bestScore = -99, cm;
    while ((cm = calRe.exec(t)) !== null) {
      const before = t.slice(Math.max(0, cm.index - 40), cm.index);
      let score = 0;
      if (/\b(want|like|prefer|be|being|target|goal|aim|make it|instead|for|to)\s*(around|about|roughly|at)?\s*$/.test(before)) score += 2;
      if (/\b(around|about|roughly)\s*$/.test(before)) score += 1;
      if (/\b(currently|current|right now|set to|at the moment|today i'?m|i'?m at)\b/.test(before)) score -= 3;
      if (cm[2]) score += 1; // followed by "cal/calories" — clearly a calorie figure
      if (score >= bestScore) { bestScore = score; best = Number(cm[1]); } // ties -> later mention wins
    }
    if (best && best >= 1000 && best <= 4500) target_calories = best;
  }

  const emphasis = /high\s*[- ]?protein/.test(t) ? "high_protein" : null;

  if (direction !== "maintain" && !(amount > 0)) {
    return { direction, amount: null, amount_unit, timeframe_weeks, target_calories, emphasis, incomplete: !target_calories };
  }
  return { direction, amount, amount_unit, timeframe_weeks, target_calories, emphasis, incomplete: false };
}
  ...
  try {
    ...
    for (const f of uf?.data || []) {
      ...
      if (f.kind === "constraint") {
        ...
        // Shapes COACHING (meal timing, pre/post-workout) — NEVER the calorie math.
        // activity_level alone drives maintenance; counting workouts here too would
        // double-count the same exercise.
        lines.push(`Activity/training: ${f.value} (use for meal timing and fueling advice ONLY — calorie targets already account for activity level. If this fact names specific days, apply it ONLY on those days. If it only states a frequency with no days, NEVER assert it is happening today — ask ("is today a training day?") or hedge ("if you're lifting today..."))`);
      } else {
...
Valid kinds: dietary_style (vegan, vegetarian, pescatarian, keto...), allergen, intolerance, restriction (foods they always avoid, e.g. "no pork"), love, dislike, health_condition, nutrient (include "frequency_per_week" if stated, e.g. ginger 5x/week), lifestyle (e.g. "works nights, skips breakfast"), activity (sports/training patterns — put schedule and frequency INSIDE the value, e.g. "hockey Tuesday evenings, 2x/week", "weight lifting 4x/week", "training for a half marathon"; if the user gives a frequency but no days, ask in your prose which days it usually happens — optional for them, but it makes fueling plans smarter), constraint (time-bounded commitment — include "expires_at" as YYYY-MM-DD if a timeframe was given).
...
USER AUTHORITY (absolute): Preference and profile decisions belong to the USER, not you. You may give your professional view briefly ONCE — but if they confirm, restate, or insist, comply immediately and emit the block(s). NEVER refuse or debate a request to add something to their profile. If asked for a LIST to add (e.g. "give me whole-foods staples and add them to my profile"), generate the list AND emit ONE batch FACT_DATA block for it in the SAME reply — never make them dictate foods back to you, never answer with philosophy instead of the card.
REFINEMENTS: when the user MODIFIES an existing stored fact ("make whole foods 80/20", "actually just twice a week"), emit BOTH blocks in one reply: <<<FACT_REMOVE>>> for the old stored value AND <<<FACT_DATA>>> for the refined value (e.g. remove "whole foods", add "whole foods, 80/20 flexible"). The user confirms each card.
...
// [v93] COMPLETED claims only. "When you tell me, I'll add those to your profile"
// is an OFFER — correcting it made the app look broken. Only "I'm adding / I've
// added / saved / locked in" (things asserted as done) trigger the correction.
const PROFILE_SAVE_CLAIM_RE = /\b(?:i(?:'m| am)\s+(?:adding|saving|updating)|i(?:'ve| have)\s+(?:added|saved|updated)|(?:added|saved|updated)\b[^.!?\n]{0,30}\bto\b|lock(?:ed)\s+(?:\w+\s+)?in)\b[^.!?\n]{0,50}\byour profile\b|\badding (?:all of )?(?:those|these|them|it|that) to your profile\b/i;
...
async function enrichIntelligenceBlocks(reply, userId) {
  ...
  // [v93] BACKSTOP: the AI once claimed "I'm adding those to your profile now" while
...
export async function POST(req) {
  try {
   ...
   const totals = sumMeals(todayMeals);          // EATEN totals
    ...
    // ═══ [v93] FOOD LOG — CODE-OWNED, EARLY RETURN ═══════════════════════════
    ...
    if (context?.type === "food_log") {
      ...
      // [v93] Decide the target date ONCE, from THIS log message ("yesterday" in the
      ...
      for (const seg of segs) {
        ...
        console.log(`  [v93] [${seg.meal_type}] "${seg.text}" -> ${items.length} food(s)${unresolved.length ? `, unresolved: ${unresolved.join(", ")}` : ""}`);
      }
...
5. 1-2 sentences. Warm and specific. No fluff, no emoji, no bullet points.`;
    ...
    // ═══ [v93] GOAL STATEMENT — CODE-OWNED, EARLY RETURN ═════════════════════
    ...
    if (!(images?.length > 0) && context?.type !== "photo" && context?.type !== "food_log") {
      ...
      // [v93] GOAL FOLLOW-UP: "No, I wanted it to be around 2000" after a goal card.
      ...
      // Still zero AI.
      if (!goalParams && context?.type === "goal_followup") {
        const numMatch = (message || "").match(/\b([12]\d{3})\b/);
        if (numMatch) {
          try {
            const { data: lastGoalRows } = await supabase.from("ai_messages")
              .select("response").eq("user_id", activeUserId)
              .like("response", "%<<<GOAL_DATA>>>%")
              .order("created_at", { ascending: false }).limit(1);
            const lastBlock = lastGoalRows?.[0]?.response?.match(/<<<GOAL_DATA>>>\s*([\s\S]*?)\s*<<<END_GOAL_DATA>>>/);
            const prior = lastBlock ? parseLooseJSON(lastBlock[1]) : null;
            goalParams = {
              direction: prior?.direction || "lose",
              amount: Number(prior?.amount) > 0 ? Number(prior.amount) : null,
              amount_unit: prior?.amount_unit || "lb",
              timeframe_weeks: null,
              target_calories: Number(numMatch[1]),
              emphasis: null,
              incomplete: false,
            };
            console.log(`=== GOAL FOLLOW-UP [v93] | requested target ${numMatch[1]}`);
          } catch (e) {}
        }
      }
      ...
      if (goalParams) {
        ...
        if (goalParams.incomplete && !goalParams.target_calories) {
        ...
        if (missing.length > 0) {
          ...
          console.log(`=== GOAL [v93] | blocked, missing: ${missing.join(", ")}`);
        ...
        if (computed) {
          ...
          console.log(`=== GOAL [v93] | code-owned | maintenance ${computed.maintenance} -> target ${computed.calories}`);
...
Local Time: ${hour}:${String(typeof localMinutes === "number" ? localMinutes : 0).padStart(2, "0")} (${timeOfDay})
...
══════════════════════════════════════════
QUESTIONS AND COMMENTS GET ANSWERS, NOT BLOCKS: when the user asks a QUESTION ("why are hand rolls high in calories", "how much protein in salmon") OR makes a COMMENT/REACTION about food ("wow that's a lot of calories", "handrolls are so heavy", "that's more than I thought"), respond in PLAIN TEXT only — explain or discuss. NEVER emit a meal block or MEAL_DATA in reply to a question or comment. Meal blocks exist ONLY when the user STATES they ate/plan to eat a specific food ("I had X", "log X"), or explicitly asks you to build a meal/plan. When in doubt, answer in plain text — the user can always tap to log.
...
THIS IS NOT OPTIONAL for single-label responses. Every nutrition-label photo response ends with MEAL_DATA.`;
    ...
    // ═══ [v93] INTELLIGENCE: inject the code-assembled PROFILE slot + capture rules.
    ...
    console.log(`=== AI [v93] | ${provider} | ${userName} | ${hour}:00 | Goal: ${goal.calories} cal | Photos: ${images?.length || 0} | Events: ${events.length}`);
